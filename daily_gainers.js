...
const getPairs = async (web3, tokenAddress, tokenSymbol, factoryABI, factoryAddress) => {
    ...
    let pairBaseNodes = [ { address: afx.get_weth_address(), symbol: afx.get_chain_symbol() }, { address: afx.get_usdt_address(), symbol: 'USDT' }, { address: afx.get_usdc_address(), symbol: 'USDC' } ]
...
const calculateTokenPrice_UniswapV2 = async (web3, pairsResult, tokenAddress, tokenDecimal, callback) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        for (const pairData of pairsResult) {
            ...
            const pairContract = new web3.eth.Contract(afx.get_uniswapv2_pool_abi(), pairData.address);
            ...
            if (pairData.base === afx.get_chain_symbol()) {
...
\tfrom ${utils.getTimeStringUTC(startDate)} UTC`
    ...
    if (!startDate) {
        ...
        message = `${process.env.BOT_TITLE} Daily Top Gainers 🤖\n`

    } else {

        message = `${process.env.BOT_TITLE} Top Gainers on historical database 🤖\n`
    }
    ...
    for (const node of result) {
        itemNum++
        const msgItem = `\n${itemNum}: <a href="https://dexscreener.com/${afx.get_dexscreener_name()}/${node.pairAddress}"><u>${node.tokenName}</u></a> (${utils.roundDecimal(node.fX, 1)}X) | MC: $${utils.roundDecimal(node.marketCap, 0)}`
        message += msgItem
    }
...
<a href="https://dexscreener.com/${afx.get_dexscreener_name()}/0x695051b0028d02172d0204c964b293d7b25b6710"><u>Chart</u></a>`
