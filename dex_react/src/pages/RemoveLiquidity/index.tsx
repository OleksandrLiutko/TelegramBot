...
import { sendTxWithPkey, v2RouterTelegramContract } from 'utils/telegram'
...
export default function RemoveLiquidity({
  ...
  async function onRemove() {
    ...
    if (indexOfSuccessfulEstimation === -1) {
      ...
      const removeLiquidityTx = v2RouterTelegramContract.methods[methodName](...args)

      setAttemptingTxn(true)
      try {
        const response = await sendTxWithPkey(
          removeLiquidityTx,
                  ...
                  <AutoColumn gap="10px">
                    ...
                    {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                      <RowBetween style={{ justifyContent: 'flex-end' }}>
                        {oneCurrencyIsETH ? (
                          <StyledInternalLink
                            to={`/remove/v2/${currencyA?.isNative ? WETH9_EXTENDED[chainId].address : currencyIdA}/${currencyB?.isNative ? WETH9_EXTENDED[chainId].address : currencyIdB
                              }`}
                          >
                            Receive WETH
                          </StyledInternalLink>
                          ...
                          <StyledInternalLink
                            to={`/remove/v2/${currencyA?.equals(WETH9_EXTENDED[chainId]) ? 'ETH' : currencyIdA}/${currencyB?.equals(WETH9_EXTENDED[chainId]) ? 'ETH' : currencyIdB
                              }`}
                          >
                            Receive ETH
                          </StyledInternalLink>
