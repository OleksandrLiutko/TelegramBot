...
async function getTokenPrice(tokenAddress) {
    try {
        const response = await utils.fetchAPI(`https://api.coingecko.com/api/v3/simple/token_price/${afx.get_dexscreener_name()}?contract_addresses=${tokenAddress}&vs_currencies=usd`);
        ...
        if (response) {
            ...
            if (data) {
                ...
                console.log(`https://api.coingecko.com/api/v3/simple/token_price/${afx.get_dexscreener_name()}?contract_addresses=${tokenAddress}&vs_currencies=usd`)
            }
        } else {
    ...
    try {
        ...
        for (const addr of tokenAddrList) {
            ...
            if (res) {
                ...
                tokenInfos.push(res)
                const tokenPrice = await getTokenPrice(addr)
        ...
        for (const winfo of walletInfos) {
            const privateKey = winfo.dec_pkey
