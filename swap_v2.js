...
export const buyToken = async (web3, database, session, tokenAddress, buyAmount, unit, ver, sendMsg, callback = null) => {
    ...
    let slippage = session.wallets[session.wallets_index].snipe_buy_slippage ? session.wallets[session.wallets_index].snipe_buy_slippage : 5;
    ...
    try {
        ...
        try {
            ...
            estimatedGas = session.wallets[session.wallets_index].snipe_max_gas_limit > uniconst.DEFAULT_ETH_GAS ? session.wallets[session.wallets_index].snipe_max_gas_limit : uniconst.DEFAULT_ETH_GAS
        }
    ...
    🚫 Your ${afx.get_chain_symbol()} balance: ${utils.roundDecimal(rawEthBalance / 10 ** 18, 5)} ${afx.get_chain_symbol()}`)
        ...
        console.log("rawEthAmount", rawEthAmount)
...
export const sellToken = async (web3, database, session, tokenAddress, sellAmount, unit, ver, sendMsg, callback = null) => {
    ...
    try {
        slippage = session.wallets[session.wallets_index].snipe_sell_slippage ? session.wallets[session.wallets_index].snipe_sell_slippage : 5
